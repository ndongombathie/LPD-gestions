// ==========================================================
// 📊 RapportsClients.jsx — Journal d'audit des clients spéciaux
// Version journal d'activité (Audit Log) pour le module Rapports
// Affichage des actions CRUD sur les clients spéciaux
// ==========================================================
...
// ==========================================================
// 📊 Composant principal — Rapports Clients (Audit Log)
// ==========================================================
export default function RapportsClients({ 
  ...
  // Charger les données d'audit une seule fois
  useEffect(() => {
    const loadData = async () => {
      try {
        ...
        const normalized = logsData.map((log) => ({
          ...
          action: log.action || "",
          client: log.cible_nom || log.client || "",
        ...
        if (response.stats) {
          ...
          setStats({
            total: response.stats.total || normalized.length,
    ...
    loadData();
  }, [dateDebut, dateFin, filterAction]); // Plus de debounce, plus de currentPage

  // Filtrage frontend avec recherche instantanée
  const logsFiltres = useMemo(() => {
    return logs
      .filter(log => {
        // Filtre par recherche texte
        if (recherche && recherche.trim() !== "") {
          return log.client?.toLowerCase().includes(recherche.toLowerCase());
        }
        return true;
      })
  ...
  const exportPDF = async () => {
    try {
      ...
      doc.setFont("helvetica", "bold");
      doc.setFontSize(13);
      doc.text("Journal d'audit des clients spéciaux", 14, y);
      ...
      // Ligne 2 : Créations et Date de début
      lineY += 5;
      ...
      // Ligne 3 : Modifications et Date de fin
      lineY += 5;
      ...
      autoTable(doc, {
        ...
        head: [["Date", "Action", "Client", "Utilisateur", "Détails"]],
      ...
      const fileName = `Journal_Audit_Clients_${todayISO()}.pdf`;
      doc.save(fileName);
