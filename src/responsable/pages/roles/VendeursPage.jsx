      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-3">
        ...
        <div className="bg-white border border-gray-200 rounded-lg p-3">
          ...
          <div className="space-y-1.5">
            {[...vendeurs]
              .sort((a, b) => (b.stats?.montantTotal || 0) - (a.stats?.montantTotal || 0))
              .slice(0, 4)
              .map((vendeur, index) => (
                <div key={vendeur.id} className="flex items-center justify-between p-2 hover:bg-gray-50 rounded">
                  <div className="flex items-center gap-2">
                    ...
                    <div>
                      <div className="text-xs font-medium text-gray-800 truncate max-w-[100px]">{vendeur.name}</div>
                      <div className="text-[10px] text-gray-500">{vendeur.stats?.objectifAtteint || 0}% objectif</div>
                    </div>
                  </div>
          ...
          <div className="grid grid-cols-2 gap-2">
            <div className="bg-gray-50 rounded p-2">
              <div className="flex items-center gap-1 mb-1">
                ...
                <span className="text-xs text-gray-600">Produits/vendeur</span>
              </div>
              <div className="text-sm font-bold text-blue-600">
                {Math.round(vendeurs.reduce((sum, v) => sum + (v.stats?.produitsVendus || 0), 0) / vendeurs.length)}
              </div>
            </div>
            <div className="bg-gray-50 rounded p-2">
              <div className="flex items-center gap-1 mb-1">
                <Clients className="w-3 h-3 text-purple-600" />
                <span className="text-xs text-gray-600">Clients/vendeur</span>
              </div>
              <div className="text-sm font-bold text-purple-600">
                {Math.round(vendeurs.reduce((sum, v) => sum + (v.stats?.clientsServis || 0), 0) / vendeurs.length)}
              </div>
            </div>
            <div className="bg-gray-50 rounded p-2">
              <div className="flex items-center gap-1 mb-1">
                <TrendingUp className="w-3 h-3 text-emerald-600" />
                <span className="text-xs text-gray-600">Ticket moyen</span>
              </div>
              <div className="text-sm font-bold text-emerald-600">
                {formatFCFA(vendeurs.reduce((sum, v) => sum + (v.stats?.ticketMoyen || 0), 0) / vendeurs.length)}
              </div>
            </div>
            <div className="bg-gray-50 rounded p-2">
              <div className="flex items-center gap-1 mb-1">
              ...
              <div className="text-sm font-bold text-orange-600">
                {Math.round(vendeurs.reduce((sum, v) => sum + (v.stats?.joursActifs || 0), 0) / vendeurs.length)}
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="bg-white border border-gray-200 rounded-lg p-3">
          ...
          <div className="space-y-2">
            {['90-100%', '80-89%', '70-79%', '<70%'].map((range, idx) => {
              const count = vendeurs.filter(v => {
                const taux = v.stats?.tauxReussite || 0;
                if (range === '90-100%') return taux >= 90;
                if (range === '80-89%') return taux >= 80 && taux < 90;
                if (range === '70-79%') return taux >= 70 && taux < 80;
                return taux < 70;
              }).length;
              
              const percentage = (count / vendeurs.length) * 100;
