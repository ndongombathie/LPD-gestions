      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-3">
        ...
        <div className="bg-white border border-gray-200 rounded-lg p-3">
          <div className="flex items-center justify-between mb-2">
            <h4 className="text-xs font-semibold text-gray-800 flex items-center gap-1">
              ...
              Classement produits gérés
            </h4>
            <TrendingUp className="w-3 h-3 text-yellow-600" />
          </div>
          <div className="space-y-1.5">
            {[...gestionnaires]
              .sort((a, b) => (b.stats?.produitsGerés || 0) - (a.stats?.produitsGerés || 0))
              .slice(0, 4)
              .map((gestionnaire, index) => (
                <div key={gestionnaire.id} className="flex items-center justify-between p-2 hover:bg-gray-50 rounded">
                  <div className="flex items-center gap-2">
                    ...
                    <div>
                      <div className="text-xs font-medium text-gray-800 truncate max-w-[100px]">{gestionnaire.name}</div>
                      <div className="text-[10px] text-gray-500">{gestionnaire.boutique}</div>
                    </div>
                  </div>
                  <div className="text-right">
                    <div className="text-xs font-semibold text-blue-600">
                      {gestionnaire.stats?.produitsGerés || 0} produits
                    </div>
                  </div>
                </div>
              ))}
          ...
          <div className="grid grid-cols-2 gap-2">
            <div className="bg-gray-50 rounded p-2">
              <div className="flex items-center gap-1 mb-1">
                ...
                <span className="text-xs text-gray-600">Transferts/mois</span>
              </div>
              <div className="text-sm font-bold text-blue-600">
                {Math.round(globalStats.transfertsReçusTotal / gestionnaires.length)}
              </div>
            </div>
            <div className="bg-gray-50 rounded p-2">
              <div className="flex items-center gap-1 mb-1">
                <Clock className="w-3 h-3 text-purple-600" />
                <span className="text-xs text-gray-600">Inventaires</span>
              </div>
              <div className="text-sm font-bold text-purple-600">
                {Math.round(globalStats.inventairesTotal / gestionnaires.length)}
              </div>
            </div>
            <div className="bg-gray-50 rounded p-2">
              <div className="flex items-center gap-1 mb-1">
            ...
            <div className="bg-gray-50 rounded p-2">
              <div className="flex items-center gap-1 mb-1">
                ...
                <span className="text-xs text-gray-600">Anomalies/mois</span>
              </div>
              <div className="text-sm font-bold text-orange-600">
                {Math.round(globalStats.anomaliesTotal / gestionnaires.length)}
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="bg-white border border-gray-200 rounded-lg p-3">
          ...
          <div className="space-y-2">
            {['95-100%', '90-94%', '85-89%', '<85%'].map((range, idx) => {
              const count = gestionnaires.filter(g => {
                const taux = g.stats?.tauxValidation || 0;
                if (range === '95-100%') return taux >= 95;
                if (range === '90-94%') return taux >= 90 && taux < 95;
                if (range === '85-89%') return taux >= 85 && taux < 90;
                return taux < 85;
              }).length;
              
              const percentage = (count / gestionnaires.length) * 100;
